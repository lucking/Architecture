...
@interface XMLDictionar () <NSXMLParserDelegate>
...
+ (XMLDictionar *)sharedInstance
{
	...
	static XMLDictionar *sharedInstance;
	dispatch_once(&once, ^{

		sharedInstance = [[XMLDictionar alloc] init];
	});
...
{
	XMLDictionar *copy = [[[self class] allocWithZone:zone] init];
...
#pragma mark  //======================="  NSDictionary(XMLDictionary)  "=================================

@implementation NSDictionary(XMLDictionary)

+ (NSDictionary *)dictionaryWithXMLParser:(NSXMLParser *)parser
{
	return [[[XMLDictionar sharedInstance] copy] dictionaryWithParser:parser];
}

+ (NSDictionary *)dictionaryWithXMLData:(NSData *)data
{
	return [[[XMLDictionar sharedInstance] copy] dictionaryWithData:data];
}

+ (NSDictionary *)dictionaryWithXMLString:(NSString *)string
{
	return [[[XMLDictionar sharedInstance] copy] dictionaryWithString:string];
}

+ (NSDictionary *)dictionaryWithXMLFile:(NSString *)path
{
	return [[[XMLDictionar sharedInstance] copy] dictionaryWithFile:path];
}
...
- (NSString *)innerXML
{
	...
	for (NSString *key in childNodes)
		{
		[nodes addObject:[XMLDictionar XMLStringForNode:childNodes[key] withNodeName:key]];
		}
...
- (NSString *)XMLString
{
	if ([self count] == 1 && ![self nodeName])
		...
		{
		return [XMLDictionar XMLStringForNode:self withNodeName:[self nodeName] ?: @"root"];
		}
}
...
#pragma mark  //======================="  NSString(XMLDictionary)  "=================================
